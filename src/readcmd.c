...
{
	static struct cmdline *static_cmdline = 0; /*déclare un pointeur vers une instance cmdline, pointeur lui-même initialisé à zéro*/
	...
	line = readline();
	if (line == NULL) { /*si aucun caractère n'est donné en entrée standard*/
		if (s) {
			freecmd(s);
			free(s);
		}
	...
	words = split_in_words(line); /*parsage de la ligne en mots*/
	free(line);
